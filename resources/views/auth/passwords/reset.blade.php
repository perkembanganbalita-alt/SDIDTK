        ...
        <div class="mt-20 flex flex-col items-center max-w-md text-center z-10">
            <div class="w-64 h-64 bg-green-500/30 rounded-full flex items-center justify-center mb-8 relative">
                <div class="w-56 h-56 bg-green-400/40 rounded-full flex items-center justify-center relative shadow-xl">
                    <svg class="w-32 h-32 text-white" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M12 15v2m-6 4h12a2 2 0 002-2v-6a2 2 0 00-2-2H6a2 2 0 00-2 2v6a2 2 0 002 2zm10-10V7a4 4 0 00-8 0v4h8z"></path></svg>
                </div>
            </div>
            
            <h2 class="text-3xl font-bold mb-4 leading-tight">Buat Kata Sandi Baru</h2>
            <p class="text-green-50 mb-10 text-sm leading-relaxed">
                Pastikan Anda menggunakan kata sandi yang kuat dan mudah diingat untuk keamanan akun Anda.
            </p>
        </div>
    ...
    <div class="w-full lg:w-1/2 flex flex-col justify-between p-8 sm:p-12 lg:p-16 xl:p-24 bg-white relative">
        <div class="max-w-md w-full mx-auto my-auto">
            <div class="mb-8">
                ...
                <p class="text-slate-500 text-sm">Masukkan kata sandi baru untuk akun Anda.</p>
            </div>

            @if($errors->any())
                <div class="mb-6 bg-red-50 border border-red-200 text-red-600 px-4 py-3 rounded-xl flex items-start gap-3">
            ...
            <form class="space-y-5" action="{{ route('password.update') }}" method="POST">
                ...
                <div>
                    ...
                    <div class="relative">
                        ...
                        <input id="email" name="email" type="email" autocomplete="email" required readonly class="appearance-none rounded-lg relative block w-full pl-4 pr-4 py-3 border border-slate-200 bg-slate-50 text-slate-500 focus:outline-none sm:text-sm transition-all shadow-sm" value="{{ $email ?? old('email') }}">
                    </div>
                </div>
                ...
                <div>
                    ...
                    <div class="relative" x-data="{ show: false }">
                        ...
                        <input id="password" name="password" :type="show ? 'text' : 'password'" required class="appearance-none rounded-lg relative block w-full pl-4 pr-10 py-3 border border-slate-200 placeholder-slate-400 text-slate-900 focus:outline-none focus:ring-2 focus:ring-[#4CA861] focus:border-[#4CA861] sm:text-sm transition-all shadow-sm" placeholder="Minimal 8 karakter">
                ...
                <div>
                    ...
                    <div class="relative" x-data="{ show: false }">
                        ...
                        <input id="password_confirmation" name="password_confirmation" :type="show ? 'text' : 'password'" required class="appearance-none rounded-lg relative block w-full pl-4 pr-10 py-3 border border-slate-200 placeholder-slate-400 text-slate-900 focus:outline-none focus:ring-2 focus:ring-[#4CA861] focus:border-[#4CA861] sm:text-sm transition-all shadow-sm" placeholder="Ulangi password baru">
                        <button type="button" @click="show = !show" class="absolute inset-y-0 right-0 pr-3 flex items-center text-slate-400 hover:text-slate-600">
                            <svg x-show="!show" class="h-5 w-5" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M15 12a3 3 0 11-6 0 3 3 0 016 0z"></path><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M2.458 12C3.732 7.943 7.523 5 12 5c4.478 0 8.268 2.943 9.542 7-1.274 4.057-5.064 7-9.542 7-4.477 0-8.268-2.943-9.542-7z"></path></svg>
                            <svg x-show="show" x-cloak class="h-5 w-5" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M13.875 18.825A10.05 10.05 0 0112 19c-4.478 0-8.268-2.943-9.543-7a9.97 9.97 0 011.563-3.029m5.858.908a3 3 0 114.243 4.243M9.878 9.878l4.242 4.242M9.88 9.88l-3.29-3.29m7.532 7.532l3.29 3.29M3 3l3.59 3.59m0 0A9.953 9.953 0 0112 5c4.478 0 8.268 2.943 9.543 7a10.025 10.025 0 01-4.132 5.411m0 0L21 21"></path></svg>
                        </button>
